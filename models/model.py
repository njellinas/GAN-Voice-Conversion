...
from models.modules import discriminator, generator_gatedcnn
...
class CycleGAN(object):
    ...
    def build_model(self):
        ...
        self.generation_B = self.generator(inputs=self.input_A_real, dim=self.num_features, reuse=False,
                                           scope_name='generator_A2B')
        self.cycle_A = self.generator(inputs=self.generation_B, dim=self.num_features, reuse=False,
                                      scope_name='generator_B2A')

        self.generation_A = self.generator(inputs=self.input_B_real, dim=self.num_features, reuse=True,
                                           scope_name='generator_B2A')
        self.cycle_B = self.generator(inputs=self.generation_A, dim=self.num_features, reuse=True,
                                      scope_name='generator_A2B')

        self.generation_A_identity = self.generator(inputs=self.input_A_real, dim=self.num_features, reuse=True,
                                                    scope_name='generator_B2A')
        self.generation_B_identity = self.generator(inputs=self.input_B_real, dim=self.num_features, reuse=True,
                                                    scope_name='generator_A2B')
        ...
        # Reserved for test
        self.generation_B_test = self.generator(inputs=self.input_A_test, dim=self.num_features, reuse=True,
                                                scope_name='generator_A2B')
        self.generation_A_test = self.generator(inputs=self.input_B_test, dim=self.num_features, reuse=True,
                                                scope_name='generator_B2A')
